...
- name: Touch a file with content
  ansible.builtin.file:
    path: /etc/foo.conf
    state: touch
    content: "any123"
...
def execute_touch(path, follow, timestamps, content):
    b_path = to_bytes(path, errors='surrogate_or_strict')
    prev_state = get_state(b_path)
    changed = False
    result = {'dest': path}
...
def main():
    ...
    if state == 'file':
        ...
        result = execute_touch(path, follow, timestamps, content)
    elif state == 'absent':
